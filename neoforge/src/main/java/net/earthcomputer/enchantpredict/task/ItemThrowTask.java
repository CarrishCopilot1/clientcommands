...
/**
 * Ported item-throw task. The original used a brigadier suggestions "fence" to wait for
 * server acknowledgement of throw packets; here we use a simple tick-based fence which is
 * less precise but adequate for client-side prediction.
 */
public abstract class ItemThrowTask extends SimpleTask {
    ...
    private final Set<PlayerRandCracker.ThrowItemsResult.Type> errorTypesHappened =
        EnumSet.noneOf(PlayerRandCracker.ThrowItemsResult.Type.class);

    public ItemThrowTask(int itemsToThrow) { this(itemsToThrow, 0); }
    ...
    protected void onTick() {
        ...
        while (((flags & FLAG_URGENT) != 0 || itemThrowsAllowedThisTick >= 1) && sentItemThrows < totalItemsToThrow) {
            ...
            if (hadUnexpectedRNGCall) return;
            if (!throwItemsResult.isSuccess()) {
                onFailedToThrowItem(throwItemsResult);
                if ((flags & FLAG_WAIT_FOR_ITEMS) != 0) return;
                failed = true;
                _break();
                return;
            }
        ...
        if (!waitingFence && sentItemThrows == totalItemsToThrow && confirmedItemThrows < sentItemThrows) {
            waitingFence = true;
            // Simple tick-based fence: in two ticks assume server has caught up.
            CompletableFuture.runAsync(() -> {
                try { Thread.sleep(100); } catch (InterruptedException ignored) {}
            }).thenRun(() -> {
                if (sentItemThrows > confirmedItemThrows) {
                    LOGGER.info("Server appears to have lost {} item throws. Rethrowing them.", sentItemThrows - confirmedItemThrows);
    ...
    @Override
    public Set<Object> getMutexKeys() { return MUTEX_KEYS; }
    ...
    protected boolean requireCrackedRNG() { return true; }

    /** Invoked from the {@code ClientPacketListenerMixin} when an item entity spawns near the player. */
    public static void handleItemSpawn(ClientboundAddEntityPacket packet) {
        if (packet.getType() != EntityType.ITEM) return;
        ...
        if (player.getEyePosition().distanceToSqr(packet.getX(), packet.getY(), packet.getZ()) > 1) return;
    ...
    private static void handleRNGCallEvent(PlayerRandCracker.RNGCallEvent event) {
        ...
        if (task != null) {
            if (task.isThrowingItem && event.getType() == PlayerRandCracker.RNGCallType.DROP_ITEM) {
                if (task.requireCrackedRNG()) event.setMaintained();
                else event.setMaintainedEvenIfSeedUnknown();
